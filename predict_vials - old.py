...
class PredictVial:
    ...
    def predict(self, images_path):
        self.searchFlag_arr = [True, True, True]
        start = time.time()
        images_arr = os.listdir(images_path)
        result = self.vials_model.detect_vial(images_path)
        # images_arr = [os.path.join(images_path, f) for f in os.listdir(images_path)]
        # pool = ThreadPool(4)
        ...
        start = time.time()
        for vial in result:
            self.getPredictExportFileItem(vial[0])
            string_ocr = self.ocr_space_file(filename=vial[2], language='eng')
            ocr_result = self.matching_strings(string_ocr)
            if ocr_result:
                stop = time.time()
                print("image of predict = " + vial[2])
                print("Vial predicted: ", vial[0])
...
if __name__ == "__main__":
    images_path = r'C:\Users\itzik.k\Desktop\Vials\Test\1005'
    predict = PredictVial()
