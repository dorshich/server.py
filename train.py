...
class Training:
    ...
    def __init__(self, yaml_path):
        self.opt = self.parse_opt(yaml_path)
    ...
    def parse_opt(self, yamel_path, known=False):
        parser = argparse.ArgumentParser()
        parser.add_argument('--weights', type=str, default=ROOT / 'yolov5s.pt', help='initial weights path')
        parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
        parser.add_argument('--data', type=str, default=ROOT / yamel_path, help='dataset.yaml path')
    ...
    def main(self, opt, callbacks=Callbacks()):
        ...
        # Evolve hyperparameters (optional)
            ...
            meta = {
                ...
                'hsv_h': (0, 0.0, 0.0),  # image HSV-Hue augmentation (fraction)
                'hsv_s': (0, 0.0, 0.0),  # image HSV-Saturation augmentation (fraction)
                'hsv_v': (0, 0.0, 0.0),  # image HSV-Value augmentation (fraction)
                'degrees': (0, 0.0, 5.0),  # image rotation (+/- deg)
                'translate': (0, 0.0, 0.0),  # image translation (+/- fraction)
                'scale': (0, 0.0, 0.0),  # image scale (+/- gain)
                'shear': (1, 0.0, 1.0),  # image shear (+/- deg)
...
if __name__ == "__main__":
    yaml_path = r'C:\Users\itzik.k\Desktop\Vials\forTraining\data.yaml'
    train = Training(yaml_path)
    train.main(train.opt)
